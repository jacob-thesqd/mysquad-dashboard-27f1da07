...
export function AuditDetail({ 
  ...
  useEffect(() => {
    if (audit) {
      ...
      onFetchTaskDetails(audit.task_id)
        .then((details) => {
            ...
            <CardContent className="pt-4 space-y-6">
              ...
              {taskDetails.checklists && taskDetails.checklists.length > 0 && (
                <div>
                  ...
                  <div className="space-y-4">
                    {taskDetails.checklists.map((checklist) => (
                      <div key={checklist.id} className="bg-muted/50 p-3 rounded-md">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                {taskDetails.custom_fields && taskDetails.custom_fields.some(field => field.name === "Go Live Date" && field.value) && (
                  <div>
                    <h4 className="font-semibold mb-1">Go Live Date</h4>
                    <p>
                      {new Date(parseInt(
                        taskDetails.custom_fields.find(field => field.name === "Go Live Date")?.value as string
                      )).toLocaleDateString()}
                    </p>
                  </div>
                )}
