...
const VirtualizedTable: React.FC<VirtualizedTableProps> = ({
  ...
  useEffect(() => {
    if (parentRef.current) {
      parentRef.current.style.height = 'calc(100% - 10px)';
    }
  }, []);

  useEffect(() => {
    if (data && data.length > 0) {
      console.log("Data for virtualization:", {
        items: data.length,
        first_item: data[0],
        parent_height: parentRef.current?.clientHeight,
        table_height: tableRef.current?.clientHeight
      });
    }
  }, [data]);
  ...
  return (
    <div ref={parentRef} className="flex-1 border rounded-md overflow-auto mb-2 h-full">
      <div 
        style={{
